/**
 * Ported 1:1 from google-gemini/gemini-cli's
 * packages/cli/src/ui/themes/theme.ts (Apache-2.0). Logic unchanged; only two
 * adaptations were required to compile outside gemini-cli's monorepo:
 *
 * 1. `CustomTheme` is defined locally instead of imported from
 *    `@google/gemini-cli-core` (a package Deus does not depend on). The shape
 *    is reconstructed exactly from how `createCustomTheme`/`validateCustomTheme`
 *    below actually read it — both the nested (`text.primary`, `background.diff.added`,
 *    ...) and legacy flat (`Foreground`, `DiffAdded`, ...) field families donor
 *    custom-theme configs may use.
 * 2. The four opacity constants come from a local, theme-scoped `./constants.js`
 *    (see that file's header comment) instead of gemini-cli's UI-wide
...
/**
 * Local replacement for `@google/gemini-cli-core`'s `CustomTheme` type (Deus
 * does not depend on that package). Reconstructed from field-by-field usage in
 * `createCustomTheme`/`validateCustomTheme` below: donor custom-theme configs
 * may use either the nested `SemanticColors`-shaped fields or the legacy flat
 * `ColorsTheme`-shaped fields (both read with `??` fallbacks throughout), so
 * both families are represented here, all optional except `name`.
 */
export interface CustomTheme {
  ...
  // Legacy flat ColorsTheme-shaped fields, still accepted for backward
  // compatibility with hand-written theme JSON.
  Background?: string;
  Foreground?: string;
  LightBlue?: string;
  AccentBlue?: string;
