...
- **Non-blocking Design** - Uses Deus scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. Deus already set up and running
...
Add Parallel AI MCP servers after the deus server:
```typescript
const mcpServers: Record<string, any> = {
  deus: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__deus__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__deus__send_message
   4. Use mcp__deus__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo deus-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep deus  # macOS
# Linux: systemctl --user status deus
```
...
- Check task runs: `tail -f logs/deus.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.deus` (macOS) or `systemctl --user restart deus` (Linux)
