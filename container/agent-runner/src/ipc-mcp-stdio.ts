...
const chatJid = process.env.DEUS_CHAT_JID!;
const groupFolder = process.env.DEUS_GROUP_FOLDER!;
const isMain = process.env.DEUS_IS_MAIN === '1';
...
server.tool(
  ...
  async (args) => {
    ...
    if (args.agent_backend !== undefined)
      data.agent_backend = args.agent_backend;
...
Use available_groups.json to find the JID for a group. The folder name must be channel-prefixed: "{channel}_{group-name}" (e.g., "whatsapp_family-chat", "telegram_dev-team", "discord_general"). Use lowercase with hyphens for the group name part.`,
  ...
  async (args) => {
    ...
    const data = {
      ...
      containerConfig: args.agent_backend
