...
| `deus-native` transcript storage | `src/agent-runtimes/transcript-store.ts`                     | Durable, append-only successful-turn JSONL at `store/transcripts/deus-native/<sha256(sessionId)>.jsonl`; separate from backend session refs and LangGraph checkpoint state |
...
| TUI backends (archived)     | `legacy/tui-phase1` branch                                        | Rust TUI's `Backend` strategy trait — archived (LIA-389); see [`docs/decisions/tui-archival.md`](docs/decisions/tui-archival.md)                                                                                 |
| Container `deus-native` portability | `container/agent-runner/src/deus-native-backend.ts`, `runtime-types.ts` | Direct, non-default `deus-native` turns over the existing container IPC/marker contract (LIA-423/F1); protocol portability only — not a production `ContainerRuntime` route |
| Container `deus-native` skill loading | `container/agent-runner/src/skill-context-loader.ts` | Lazy instruction-pack discovery/injection for the F1 container driver (LIA-426/F4) — a `load_skill` tool plus direct `/name` invocation; distinct from `skill-mcp-registry.ts`'s executable MCP-tool registration |
...
| Linear dispatcher           | `src/linear-dispatcher.ts`                                        | Polls Linear for "Ready for Agent" issues, runs container agents with role prompts, posts results back. `deus-native` is a selectable backend but refuses real coding dispatch via a capability guard until its tool surface supports it (see `docs/decisions/linear-pipeline-deus-native-capability-boundary.md`) |
